...
from sentibert.pytorch_pretrained_bert.tokenization import BertTokenizer
...
for line in f2:
    ...
    if '(#euthanasia' in sen:
        data["sentences"][idx]["sentimentTree"] = data["sentences"][idx]["sentimentTree"].replace('(#euthanasia',
                                                                                                  '#euthanasia')
    words = sen.split(' ')
    for word in words:
        if '(' in word:
            data["sentences"][idx]["sentimentTree"] = data["sentences"][idx]["sentimentTree"].replace(word,
                                                                                                      word.replace('(',
                                                                                                                   ''))
        word = word.replace('(', '')
        if ')' == word:
            data["sentences"][idx]["sentimentTree"] = data["sentences"][idx]["sentimentTree"].replace(
                "(X|sentiment=3|prob=0.990 )", "(X|sentiment=3|prob=0.990 .")
        else:
            if ')' in word:
                data["sentences"][idx]["sentimentTree"] = data["sentences"][idx]["sentimentTree"].replace(word,
                                                                                                          word.replace(
                                                                                                              ')', ''))
                data["sentences"][idx]["sentimentTree"] = data["sentences"][idx]["sentimentTree"].replace("U19sadODI):",
                                                                                                          "U19sadODI:")

    idx += 1

twitter_label = np.random.rand(15, 1)
# np.load(os.path.join(args.data_dir, 'twitter_' + args.stage + '_label.npy'))
expression = []
sen_set = data["sentences"]

for sen in sen_set:
    if sen[
        "sentimentTree"] != "(ROOT|sentiment=2|prob=0.915 (VP|sentiment=2|prob=0.983 1) (.|sentiment=2|prob=0.997 .))":
        expression.append(
            sen["sentimentTree"].replace("(Reuters", "Reuters").replace(":(", "sad").replace("(via", "via"))
    else:
