...
from run_classifier_dataset_utils_new import processors, output_modes, convert_examples_to_features, \
    convert_examples_to_features_phrase, compute_metrics
...
def main():
    ...
    if args.do_train:
        ...
        if task_name == "sstphrase" or task_name == "sst-3":
            train_data = TensorDataset(all_input_ids, all_input_mask, all_phrase_mask, all_segment_ids, all_label_ids,
                                       all_span, all_span_3)
        else:
            if (
                    task_name == "sst-2" or task_name == "twitter" or task_name == "emocontext" or task_name == "emoint" or task_name == "xxx") and args.para == "sentibert":
                train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_span,
                                           all_span_3)
            else:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch", disable=args.local_rank not in [-1, 0]):
            ...
            for step, batch in enumerate(
                ...
                if task_name == "sstphrase" or task_name == "sst-3":
                    ...
                    logits, loss = model(input_ids, token_type_ids=segment_ids, attention_mask=input_mask,
                                         phrase_mask=phrase_mask, graph_label=label_ids, span=span, span_3=span_3)
                else:
                    if (
                            task_name == "sst-2" or task_name == "twitter" or task_name == "emocontext" or task_name == "emoint" or task_name == "xxx") and args.para == "sentibert":
                        input_ids, input_mask, segment_ids, label_ids, span, span_3 = batch
                        logits, loss = model(input_ids, token_type_ids=segment_ids, attention_mask=input_mask,
                                             labels=label_ids, span=span, span_3=span_3)
                    else:
                ...
                if output_mode == "classification":
                    ...
                    tmp = logits.view(-1, num_labels)
                    tmp2 = label_ids.view(-1)
                    act = tmp2.cpu().detach().numpy()
                    pr = np.argmax(tmp.cpu().detach().numpy(), axis=1)
                    ac = act == pr
                    ac = sum(ac) / len(ac)
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        if task_name == "sstphrase" or task_name == "sst-3":
            model = BertForPhraseClassification.from_pretrained(args.output_dir, finetuned_model_name,
                                                                num_labels=num_labels)
        else:
            model = BertForSequenceClassification.from_pretrained(args.output_dir, finetuned_model_name,
                                                                  num_labels=num_labels)
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        if task_name == "sstphrase" or task_name == "sst-3":
            eval_data = TensorDataset(all_input_ids, all_input_mask, all_phrase_mask, all_segment_ids, all_label_ids,
                                      all_span, all_span_3)
        else:
            if (
                    task_name == "sst-2" or task_name == "twitter" or task_name == "emocontext" or task_name == "emoint" or task_name == "xxx") and args.para == "sentibert":
                eval_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_span,
                                          all_span_3)
            else:
        ...
        if task_name == "sstphrase" or task_name == "sst-3":
            for input_ids, input_mask, phrase_mask, segment_ids, label_ids, span, span_3 in tqdm(eval_dataloader,
                                                                                                 desc="Evaluating"):
            ...
            if (
                    task_name == "sst-2" or task_name == "twitter" or task_name == "emocontext" or task_name == "emoint" or task_name == "xxx") and args.para == "sentibert":
                for input_ids, input_mask, segment_ids, label_ids, span, span_3 in tqdm(eval_dataloader,
                                                                                        desc="Evaluating"):
                    input_ids = input_ids.to(device)
                    input_mask = input_mask.to(device)
                    segment_ids = segment_ids.to(device)
        ...
        if task_name == "mnli":
            ...
            eval_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_graph, all_label_ids,
                                      all_span)
            # Run prediction for full data
            eval_sampler = SequentialSampler(eval_data)
            eval_dataloader = DataLoader(eval_data, sampler=eval_sampler, batch_size=args.eval_batch_size)
